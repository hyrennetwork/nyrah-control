    ...
    fun main(args: Array<String>) {
        ServerBootstrap()
            ...
            .childHandler(object : ChannelInitializer<Channel>() {
                private var protocol = Protocol.HANDSHAKE

                override fun initChannel(
                    channel: Channel
                ) {
                    val remoteAddress = if (channel.remoteAddress() == null) {
                        channel.parent().remoteAddress()
                    } else {
                        channel.remoteAddress()
                    }
                    val pipeline = channel.pipeline()

                    pipeline.addFirst(object : MessageToMessageDecoder<ByteBuf>() {
                        override fun decode(
                            ...
                            val packetSize = byteBuf.readVarInt()

                            println("Packet Size: $packetSize")

                            val packetId = byteBuf.readVarInt()

                            println("Received packet #$packetId request")

                            val packet = protocol.TO_SERVER.createPacket(packetId) ?: throw RuntimeException(
                            ...
                            if (packet is HandshakePacket) {
                                ...
                                // Ignore this = packet size
                                byteBuf.readVarInt()

                                val packetId = byteBuf.readVarInt()

                                println("Status: #$packetId")
                                ...
                                if (packet is StatusRequestPacket) {
                                    ...
                                    val statusResponsePacket = StatusResponsePacket(response)

                                    val packetId = protocol.TO_CLIENT.getPacketId(statusResponsePacket) ?: throw RuntimeException(
                                        "Cannot send packet ${packet::class.qualifiedName}"
                                    )

                                    val packetSize = statusResponsePacket.size() + packetId.getVarIntSize()
...
//                                throw RuntimeException("Did not read all bytes from packet ${packet::class.qualifiedName} (${byteBuf.readableBytes()} bytes remaining)")
//                            }
                        }
                    })
                }
            ...
            ).localAddress(25565).bind().addListener {
                if (it.isSuccess) {
                    println("Sucesso")
                } else {
                    println("Falhou")
